...
export default function ReceptionDashboard({ profile, panel, loading, error, updatedAt, onRefresh, onNavigate, onNewStudent }) {
  ...
  const groups = Object.entries(panel.appointments.reduce((result, item) => {
    const time = timeOf(item.start_time)
    result[time] = (result[time] || 0) + 1
    return result
  }, {})).sort(([a], [b]) => a.localeCompare(b))
  const upcoming = groups.filter(([time]) => time >= clock)
  const visible = (period === 'upcoming' ? upcoming : groups).slice(0, 6)
  ...
  return <section className="rx-dashboard" aria-label="Painel da recepção" aria-busy={loading}>
    <div className="rx-intro">
      ...
      <div className="rx-update"><button className="rx-button rx-secondary" onClick={onRefresh} disabled={loading} type="button"><ManagementIcon name="refresh" size={17} />{loading ? 'Atualizando…' : 'Atualizar'}</button><span role="status">{error ? 'Atualização não concluída' : updatedAt ? `Atualizado às ${updatedAt.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}` : 'Carregando informações'}</span></div>
    </div>
    {error && <div className="rx-error" role="alert"><ManagementIcon name="alert" size={20} /><span>{error} {ready ? 'Exibindo a última atualização disponível.' : 'Tente atualizar o painel.'}</span></div>}
    ...
    <div className="rx-grid">
      <section className="rx-panel rx-agenda"><div className="rx-panel-heading"><div><span className="rx-eyebrow">FLUXO DE ATENDIMENTO</span><h3>Agenda de hoje</h3></div><button className="rx-link" onClick={() => onNavigate('Agenda')} type="button">Ver agenda<ManagementIcon name="arrow" size={16} /></button></div>
        <div className="rx-filters" role="group" aria-label="Filtrar horários">{[['upcoming', 'Próximos horários'], ['all', 'Todos de hoje']].map(([key, label]) => <button type="button" key={key} aria-pressed={period === key} onClick={() => setPeriod(key)}>{label}</button>)}</div>
        {ready && visible.length ? <div className="rx-slots">{visible.map(([time, count]) => <button className="rx-slot" type="button" key={time} onClick={() => onNavigate('Agenda')} aria-label={`Abrir agenda: ${time}, ${plural(count, 'reserva', 'reservas')}`}><time>{time}</time><span><b>Musculação</b><small>{plural(count, 'reserva confirmada', 'reservas confirmadas')}</small></span><span className="rx-tag">{time < clock ? 'Horário passado' : 'Agendado'}</span><ManagementIcon name="arrow" size={16} /></button>)}</div> : <div className="rx-empty"><span className="rx-empty-icon"><ManagementIcon name="calendar" size={26} /></span><strong>{!ready ? 'Carregando a agenda' : period === 'upcoming' ? 'Nenhum próximo horário' : 'Agenda livre por aqui'}</strong><p>{!ready ? 'Os horários aparecerão assim que os dados chegarem.' : period === 'upcoming' && groups.length ? 'Consulte “Todos de hoje” para ver os horários anteriores.' : 'As reservas confirmadas aparecerão neste painel.'}</p></div>}
        <div className="rx-agenda-foot"><ManagementIcon name="calendar" size={16} /><span>{ready ? `${plural(groups.length, 'horário com reserva', 'horários com reservas')} hoje` : 'Consultando horários'}</span>{visible.length === 6 && <span>Exibindo até 6 horários</span>}</div>
      </section>
