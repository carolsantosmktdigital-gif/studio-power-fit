...
export default function ReceptionDashboard({ profile, panel, loading, error, updatedAt, onRefresh, onNavigate, onNewStudent, onScheduleStudent, onRegisterPayment }) {
  ...
  const visibleGroups = period === 'upcoming' ? upcomingGroups.slice(0, 3) : appointmentGroups
  const groups = appointmentGroups.map(([time, items]) => [time, items.length])
  const upcoming = upcomingGroups.map(([time, items]) => [time, items.length])
