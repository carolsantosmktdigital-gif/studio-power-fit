...
function AppShell({ profile, onLogout }) {
  ...
  return <div className={`app-shell dashboard-shell ${darkMode ? 'theme-dark' : 'theme-light'} ${isAdmin ? 'is-admin' : 'is-reception'}`}>
    ...
    {quickAction?.type === 'booking' && <div className="modal-backdrop"><form className="student-modal quick-action-modal quick-booking-modal" onSubmit={saveQuickBooking}>
      ...
      <div className="quick-day-columns">{[0,1,2].map(offset=>{const d=new Date();d.setDate(d.getDate()+offset);const v=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;const visible=quickSlots.filter(s=>s.appointment_date===v&&!s.is_past);return <section className={offset===0?'quick-day-card today':'quick-day-card'} key={v}><div className="quick-day-head"><ManagementIcon name="calendar" size={21}/><div><strong>{offset===0?'Hoje':offset===1?'Amanhã':'Depois de amanhã'}</strong><span>{new Date(v+'T12:00:00').toLocaleDateString('pt-BR',{weekday:'long',day:'2-digit',month:'2-digit',year:'numeric'})}</span></div></div><div className="quick-day-slots">{quickLoading?<p>Carregando…</p>:visible.length?visible.map(s=>{const full=Number(s.available||0)<=0;return <button type="button" key={s.start_time} disabled={full} className={`${full?'full':''} ${!full&&quickAction.date===v&&quickAction.time===String(s.start_time).slice(0,5)?'active':''}`} onClick={()=>!full&&setQuickAction({...quickAction,date:v,time:String(s.start_time).slice(0,5)})}><span>{full?'●':'●'}</span><strong>{String(s.start_time).slice(0,5)}</strong>{full&&<small>Lotado</small>}</button>}):<p className="quick-no-slots">Sem horários futuros</p>}</div></section>})}</div>
