...
function AppShell({ profile, onLogout }) {
  ...
  const registerAgendaAttendance = async (item, status = 'PRESENTE') => {
    setNotice('')
    const attendanceStatus = status === 'FALTOU' ? 'AUSENTE' : 'PRESENTE'
    ...
    setNotice(status === 'PRESENTE' ? 'Presença registrada com sucesso.' : 'Falta registrada com sucesso.')
