...
function AppShell({ profile, onLogout }) {
  ...
  return <div className={`app-shell dashboard-shell ${darkMode ? 'theme-dark' : 'theme-light'} ${isAdmin ? 'is-admin' : 'is-reception'}`}>
    ...
    {editingStudent && <div className="modal-backdrop"><form className="student-modal student-profile-modal" onSubmit={saveStudent}>
      ...
      {studentTab === 'plano' && <div className="student-tab-panel"><div className="plan-panel-grid"><label>Tipo de plano<select value={editingStudent.plan_code} onChange={(event) => setEditingStudent((current) => ({ ...current, plan_code: event.target.value }))}><option value="MENSALISTA">Mensalista</option><option value="TOTALPASS">TotalPass</option><option value="WELLHUB">Wellhub</option></select></label><div className="plan-current"><strong>Plano ativo</strong><span>{formatPlan(editingStudent.plan_code)}</span></div></div>{editingStudent.plan_code === 'MENSALISTA' && <div className="finance-link-card"><div><strong>Gerencie as cobranças no Financeiro</strong><span>Criação, vencimentos, pagamentos e histórico financeiro ficam separados do cadastro.</span></div><button type="button" onClick={() => openStudentFinance(editingStudent)}>Ir para Pagamentos →</button></div>}<button className="dashboard-primary-action student-save" type="submit">Salvar plano</button></div>}
