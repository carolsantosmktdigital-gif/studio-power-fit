...
function AppShell({ profile, onLogout }) {
  ...
  const navIcon = (item) => item === 'Alunos' ? '◉' : item === 'Professores' || item === 'Funcionários' ? '♟' : item === 'Auditoria' || item === 'Relatórios' ? '◷' : item === 'Pagamentos' || item === 'Financeiro' ? '▣' : item === 'Agenda' || item === 'Agendamentos' ? '▢' : item === 'Comunicação' ? '✉' : item === 'Lista de espera' ? '◌' : '⌂'
  ...
  return <div className={`app-shell dashboard-shell ${darkMode ? 'theme-dark' : 'theme-light'} ${isAdmin ? 'is-admin' : 'is-reception'}`}>
    ...
    {mobileMenuOpen && <section className="management-mobile-sheet" aria-label="Mais opções"><div className="management-mobile-sheet-heading"><div><span>GESTÃO</span><strong>Mais opções</strong></div><button aria-label="Fechar menu" onClick={() => setMobileMenuOpen(false)} type="button">×</button></div><div className="management-mobile-sheet-grid">{mobileMoreItems.map((item) => <button key={item} className={page === navigationPage(item) ? 'active' : ''} onClick={() => navigateMobile(item)} type="button"><span>{navIcon(item)}</span><strong>{navLabel(item)}</strong></button>)}</div><button className="management-mobile-logout" onClick={onLogout} type="button"><span>⇥</span>Sair da conta</button></section>}
    <nav className="management-mobile-nav" aria-label="Navegação da gestão">{mobilePrimaryItems.map((item) => <button key={item} className={page === navigationPage(item) ? 'active' : ''} onClick={() => navigateMobile(item)} type="button"><span>{navIcon(item)}</span><small>{navLabel(item)}</small></button>)}<button className={mobileMoreItems.some((item) => page === navigationPage(item)) || mobileMenuOpen ? 'active' : ''} onClick={() => setMobileMenuOpen((current) => !current)} type="button"><span>☰</span><small>Mais</small></button></nav>
