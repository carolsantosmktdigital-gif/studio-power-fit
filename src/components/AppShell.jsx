...
function AppShell({ profile, onLogout }) {
  ...
  const openQuickBooking = async () => {
    setQuickAction({ type: 'booking', student_id: '', date: todayIso(), time: '' })
    setQuickLoading(true)
    const { data, error } = await supabase.rpc('get_student_booking_slots', { p_start_date: todayIso() })
    setQuickSlots(error ? [] : data ?? [])
    setQuickLoading(false)
  }
  const openQuickPayment = () => setQuickAction({ type: 'payment', scope: 'ALUNO', student_id: '', category: 'MENSALIDADE', amount: '', description: '', due_date: todayIso(), linked_payment_id: '' })
  ...
  return <div className={`app-shell dashboard-shell ${darkMode ? 'theme-dark' : 'theme-light'} ${isAdmin ? 'is-admin' : 'is-reception'}`}>
    ...
    <div className="app-main"><header className="app-header"><div><span className="header-kicker">STUDIO POWER FIT · DEMONSTRAÇÃO</span><h1>{title}</h1></div><div className="header-actions"><button className="header-theme-button" aria-label={darkMode ? 'Ativar modo claro' : 'Ativar modo escuro'} title={darkMode ? 'Ativar modo claro' : 'Ativar modo escuro'} onClick={() => setDarkMode(!darkMode)} type="button"><ManagementIcon name={darkMode ? 'sun' : 'moon'} size={19} /></button><div className="header-user"><div className="user-avatar">{(profile.full_name || 'A')[0]}</div><div className="user-info"><strong>{profile.full_name}</strong><span>{isAdmin ? 'Gestão' : 'Recepção'}</span></div></div></div></header><main className="app-content">
      ...
      {!isAdmin && page === 'Início' && <ReceptionDashboard profile={profile} panel={receptionPanel} loading={receptionLoading} error={receptionError} updatedAt={receptionUpdatedAt} onRefresh={loadReceptionPanel} onNavigate={setPage} onNewStudent={() => openForm('ALUNO')} onScheduleStudent={openQuickBooking} onRegisterPayment={openQuickPayment} />}
    ...
    {quickAction?.type === 'booking' && <div className="modal-backdrop"><form className="student-modal quick-action-modal" onSubmit={saveQuickBooking}><div className="panel-heading"><div><span className="placeholder-kicker">RECEPÇÃO</span><h3>Agendar aluno</h3></div><button className="modal-close" type="button" onClick={()=>setQuickAction(null)}>×</button></div><label>Aluno<select value={quickAction.student_id} onChange={e=>setQuickAction({...quickAction,student_id:e.target.value})} required><option value="">Selecione o aluno</option>{students.filter(s=>s.status==='ATIVO').map(s=><option key={s.id} value={s.id}>{s.profile?.full_name}</option>)}</select></label><div className="quick-date-tabs">{[0,1,2].map(offset=>{const d=new Date();d.setDate(d.getDate()+offset);const v=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;return <button type="button" className={quickAction.date===v?'active':''} key={v} onClick={()=>setQuickAction({...quickAction,date:v,time:''})}>{offset===0?'Hoje':offset===1?'Amanhã':'Depois de amanhã'}<span>{new Date(v+'T12:00:00').toLocaleDateString('pt-BR')}</span></button>})}</div><div className="quick-slot-grid">{quickLoading?<p>Carregando horários…</p>:quickSlots.filter(s=>s.appointment_date===quickAction.date&&!s.is_past).map(s=><button type="button" key={s.start_time} disabled={Number(s.available||0)<=0} className={quickAction.time===String(s.start_time).slice(0,5)?'active':''} onClick={()=>setQuickAction({...quickAction,time:String(s.start_time).slice(0,5)})}><strong>{String(s.start_time).slice(0,5)}</strong><span>{Number(s.available||0)>0?`${s.available} vaga(s)`:'Lotado'}</span></button>)}</div><button className="dashboard-primary-action" type="submit" disabled={quickLoading}>Confirmar agendamento</button></form></div>}
    {quickAction?.type === 'payment' && <div className="modal-backdrop"><form className="student-modal quick-action-modal" onSubmit={saveQuickPayment}><div className="panel-heading"><div><span className="placeholder-kicker">RECEPÇÃO</span><h3>Cadastrar pagamento</h3></div><button className="modal-close" type="button" onClick={()=>setQuickAction(null)}>×</button></div><div className="quick-choice"><button type="button" className={quickAction.scope==='ALUNO'?'active':''} onClick={()=>setQuickAction({...quickAction,scope:'ALUNO'})}>Aluno</button><button type="button" className={quickAction.scope==='OUTROS'?'active':''} onClick={()=>setQuickAction({...quickAction,scope:'OUTROS',student_id:'',category:'OUTROS'})}>Outros</button></div>{quickAction.scope==='ALUNO'?<><label>Aluno<select value={quickAction.student_id} onChange={e=>selectQuickPaymentStudent(e.target.value)} required><option value="">Selecione o aluno</option>{students.map(s=><option key={s.id} value={s.id}>{s.profile?.full_name}</option>)}</select></label><label>Referente a<select value={quickAction.category} onChange={e=>{const category=e.target.value;const studentId=quickAction.student_id;const nearest=payments.filter(p=>p.student_id===studentId&&p.payment_type==='MENSALIDADE'&&p.status!=='CANCELADO').sort((a,b)=>Math.abs(new Date(a.due_date)-new Date())-Math.abs(new Date(b.due_date)-new Date()))[0];setQuickAction({...quickAction,category,amount:category==='DAY_USE'?'20.00':category==='MENSALIDADE'?(nearest?.amount||''):'',linked_payment_id:category==='MENSALIDADE'?(nearest?.id||''):''})}}><option value="DAY_USE">Day use</option><option value="MENSALIDADE">Mensalidade</option><option value="OUTROS">Outros</option></select></label>{quickAction.category==='MENSALIDADE'&&<div className="quick-payment-hint">{quickAction.linked_payment_id?`Cobrança mais próxima localizada. Vencimento: ${new Date(quickAction.due_date+'T12:00:00').toLocaleDateString('pt-BR')}`:'Nenhuma mensalidade pendente localizada para este aluno.'}</div>}{quickAction.category==='OUTROS'&&<label>Descrição<input value={quickAction.description} onChange={e=>setQuickAction({...quickAction,description:e.target.value})} required /></label>}</>:<label>Descrição do pagamento<input value={quickAction.description} onChange={e=>setQuickAction({...quickAction,description:e.target.value})} required /></label>}<label>Valor (R$)<input inputMode="decimal" value={quickAction.amount} onChange={e=>setQuickAction({...quickAction,amount:e.target.value})} placeholder="0,00" required /></label><button className="dashboard-primary-action" type="submit">Cadastrar pagamento</button></form></div>}
