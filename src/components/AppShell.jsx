...
function AppShell({ profile, onLogout }) {
  ...
  const loadAgenda = async () => {
    const today = new Date().toISOString().slice(0, 10)
    const { data } = await supabase.from('appointments').select('id, student_id, teacher_id, appointment_date, start_time, status').gte('appointment_date', today).order('appointment_date').order('start_time').limit(100)
  ...
  return <div className={`app-shell dashboard-shell ${darkMode ? 'theme-dark' : 'theme-light'} ${isAdmin ? 'is-admin' : 'is-reception'}`}>
    ...
    <div className="app-main"><header className="app-header"><div><span className="header-kicker">STUDIO POWER FIT · DEMONSTRAÇÃO</span><h1>{title}</h1></div><div className="header-actions"><button className="header-theme-button" aria-label={darkMode ? 'Ativar modo claro' : 'Ativar modo escuro'} title={darkMode ? 'Ativar modo claro' : 'Ativar modo escuro'} onClick={() => setDarkMode(!darkMode)} type="button"><ManagementIcon name={darkMode ? 'sun' : 'moon'} size={19} /></button><div className="header-user"><div className="user-avatar">{(profile.full_name || 'A')[0]}</div><div className="user-info"><strong>{profile.full_name}</strong><span>{isAdmin ? 'Gestão' : 'Recepção'}</span></div></div></div></header><main className="app-content">
      ...
      {page === 'Agenda' && (()=>{const today=todayIso();const todayItems=appointments.filter(a=>a.appointment_date===today);const teacherName=(id)=>employees.find(e=>e.teacher_id===id)?.profile?.full_name||'Professor a definir';const statusLabel=(s)=>s==='CONFIRMADO'?'Aguardando presença':s==='PRESENTE'?'Presença confirmada':s==='FALTOU'?'Falta registrada':s==='CANCELADO'?'Cancelado':s;return <section className="students-page live-data-page agenda-redesign"><div className="agenda-title-row"><div><h2>Agendamentos</h2><p>Gerencie os agendamentos e acompanhe os atendimentos do dia.</p></div><button className="dashboard-primary-action" type="button" onClick={openQuickBooking}>+&nbsp; Agendar aluno</button></div><div className="agenda-filterbar"><div className="agenda-period"><button className="active" type="button">Hoje</button><button type="button" onClick={()=>setNotice('Os próximos dias já estão disponíveis pelo agendamento rápido.')}>Próximos dias</button></div><label>Data<input type="date" value={today} readOnly/></label><label>Professor<select defaultValue=""><option value="">Todos os professores</option>{employees.filter(e=>e.teacher_id&&e.teacher_status==='ATIVO').map(e=><option key={e.teacher_id} value={e.teacher_id}>{e.profile?.full_name}</option>)}</select></label><label>Status<select defaultValue=""><option value="">Todos os status</option><option>Confirmado</option><option>Presente</option><option>Falta</option><option>Cancelado</option></select></label></div><div className="agenda-summary"><article><span>◉</span><strong>{todayItems.length}</strong><small>Agendamentos hoje</small></article><article className="green"><span>✓</span><strong>{todayItems.filter(a=>a.status==='PRESENTE').length}</strong><small>Presenças confirmadas</small></article><article className="amber"><span>◷</span><strong>{todayItems.filter(a=>a.status==='CONFIRMADO').length}</strong><small>Aguardando presença</small></article><article className="red"><span>×</span><strong>{todayItems.filter(a=>a.status==='FALTOU').length}</strong><small>Faltas registradas</small></article><article><span>♙</span><strong>{receptionPanel.waitlist}</strong><small>Lista de espera</small></article></div><div className="agenda-content-grid"><div className="agenda-main-card"><div className="agenda-card-head"><div><span className="placeholder-kicker">AGENDA DO DIA</span><h3>{new Date(today+'T12:00:00').toLocaleDateString('pt-BR',{weekday:'long',day:'2-digit',month:'long',year:'numeric'})}</h3></div><button className="outline-action" onClick={loadAgenda} type="button"><ManagementIcon name="refresh" size={16}/> Atualizar</button></div><div className="agenda-list">{todayItems.length?todayItems.map(item=><article className={`agenda-row status-${String(item.status).toLowerCase()}`} key={item.id}><time>{String(item.start_time).slice(0,5)}</time><div className="agenda-person"><span>{(item.student?.full_name||'A')[0]}</span><div><strong>{item.student?.full_name||'Aluno'}</strong><small>Agendamento confirmado</small></div></div><div className="agenda-teacher"><small>PROFESSOR</small><strong>{teacherName(item.teacher_id)}</strong></div><span className={`agenda-status ${String(item.status).toLowerCase()}`}>{statusLabel(item.status)}</span><div className="agenda-actions">{item.status==='CONFIRMADO'&&<button className="agenda-presence" type="button" onClick={()=>setNotice('Use o fluxo de atendimento da página inicial para efetuar a presença.')}>Efetuar presença</button>}<button className="agenda-more" type="button">•••</button></div></article>):<div className="agenda-empty">Nenhum agendamento para hoje.</div>}</div><aside className="agenda-sidebar"><section className="agenda-calendar"><div className="agenda-calendar-head"><strong>${new Date(today+'T12:00:00').toLocaleDateString('pt-BR',{month:'long',year:'numeric'}).replace(/^./,x=>x.toUpperCase())}</strong></div><div className="agenda-weekdays">${['D','S','T','Q','Q','S','S'].map(d=><span key={d}>{d}</span>)}</div><div className="agenda-calendar-days">${Array.from({length:new Date(Number(today.slice(0,4)),Number(today.slice(5,7)),0).getDate()},(_,i)=>i+1).map(day=><button key={day} type='button' className={day===Number(today.slice(8,10))?'active':''}>{day}</button>)}</div></section><section className="agenda-side-card"><div className="agenda-side-title"><strong>Professores</strong><span>{employees.filter(e=>e.teacher_id&&e.teacher_status==='ATIVO').length} ativos</span></div>${employees.filter(e=>e.teacher_id&&e.teacher_status==='ATIVO').map(e=><div className='agenda-side-person' key={e.teacher_id}><span>{(e.profile?.full_name||'P')[0]}</span><div><strong>{e.profile?.full_name}</strong><small>{todayItems.filter(a=>a.teacher_id===e.teacher_id).length} agendamento(s)</small></div></div>)}</section><section className="agenda-side-card"><div className="agenda-side-title"><strong>Status</strong></div><div className="agenda-side-status"><span><i className='green'></i>Presença confirmada</span><span><i className='amber'></i>Aguardando presença</span><span><i className='red'></i>Falta registrada</span><span><i></i>Cancelado</span></div></section></aside></div></section>})()}
