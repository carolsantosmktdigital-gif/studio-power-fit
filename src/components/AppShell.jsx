...
function AppShell({ profile, onLogout }) {
  ...
  const loadAgenda = async () => {
    ...
    const { data } = await supabase.from('appointments').select('id, student_id, teacher_id, appointment_date, start_time, status').gte('appointment_date', today).order('appointment_date').order('start_time').limit(500)
