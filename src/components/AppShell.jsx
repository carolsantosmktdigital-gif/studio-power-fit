...
function AppShell({ profile, onLogout }) {
  ...
  const loadStudents = async () => {
    let { data, error } = await supabase.from('students').select('id, profile_id, status, payment_plan, billing_due_day, created_at').order('created_at', { ascending: false })
  ...
  const saveStudent = async (event) => {
    ...
    const { data: savedStudent, error } = await supabase.from('students').update({ status: editingStudent.status, billing_due_day: dueDay }).eq('id', editingStudent.id).select('status, billing_due_day').single()
  ...
  const openStudentEditor = (student) => {
    ...
    setStudentTab('dados')
    setEditingStudent({ ...student, plan_code: planCode, original_plan_code: planCode, billing_due_day: student.billing_due_day || '', profile: maskedProfile(student.profile) })
  }
  ...
  return <div className={`app-shell dashboard-shell ${darkMode ? 'theme-dark' : 'theme-light'} ${isAdmin ? 'is-admin' : 'is-reception'}`}>
    ...
    {editingStudent && <div className="modal-backdrop"><form className="student-modal student-profile-modal" onSubmit={saveStudent}>
      ...
      {studentTab === 'plano' && <div className="student-tab-panel"><div className="plan-panel-grid"><label>Tipo de plano<select value={editingStudent.plan_code} onChange={(event) => setEditingStudent((current) => ({ ...current, plan_code: event.target.value }))}><option value="MENSALISTA">Mensalista</option><option value="TOTALPASS">TotalPass</option><option value="WELLHUB">Wellhub</option></select></label>{editingStudent.plan_code === 'MENSALISTA' && <label>Dia do vencimento<input type="number" min="1" max="31" placeholder="Ex.: 10" value={editingStudent.billing_due_day || ''} onChange={(event) => setEditingStudent((current) => ({ ...current, billing_due_day: event.target.value }))} required /><small>A próxima cobrança será criada automaticamente 10 dias antes do vencimento.</small></label>}<div className="plan-current"><strong>Plano ativo</strong><span>{formatPlan(editingStudent.plan_code)}</span></div></div>{editingStudent.plan_code === 'MENSALISTA' && <div className="finance-link-card"><div><strong>Gerencie as cobranças no Financeiro</strong><span>Criação, vencimentos, pagamentos e histórico financeiro ficam separados do cadastro.</span></div><button type="button" onClick={() => openStudentFinance(editingStudent)}>Ir para Pagamentos →</button></div>}<button className="dashboard-primary-action student-save" type="submit">Salvar plano</button></div>}
