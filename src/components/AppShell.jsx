...
function AppShell({ profile, onLogout }) {
  ...
  const loadAgenda = async () => {
    ...
    const { data: waitPeople } = waitProfileIds.length ? await supabase.from('profiles').select('id, full_name, phone').in('id', waitProfileIds) : { data: [] }
  ...
  const copyWaitlistPhone = async (phone) => {
    if (!phone) return setNotice('Este aluno não possui telefone cadastrado.')
    try { await navigator.clipboard.writeText(phone); setNotice('Telefone copiado para a área de transferência.') }
    catch { setNotice('Não foi possível copiar o telefone.') }
  }
  ...
  const registerAgendaAttendance = async (item, status = 'REALIZADO') => {
    setNotice('')
    const attendanceStatus = status === 'FALTOU' ? 'FALTOU' : 'PRESENTE'
  ...
  const undoAgendaAttendance = async (item) => {
    ...
    setNotice(item.status === 'REALIZADO' ? 'Presença desfeita. O aluno voltou para aguardando presença.' : 'Falta desfeita. O aluno voltou para aguardando presença.')
    await Promise.all([loadAgenda(), loadReceptionPanel(), loadHealth()])
    return true
  }
  ...
  return <div className={`app-shell dashboard-shell ${darkMode ? 'theme-dark' : 'theme-light'} ${isAdmin ? 'is-admin' : 'is-reception'} ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
    ...
    <div className="app-main"><header className="app-header"><div><span className="header-kicker">STUDIO POWER FIT · DEMONSTRAÇÃO</span><h1>{title}</h1></div><div className="header-actions">{profile.role === 'RECEPCAO' && <div className="header-notification-wrap"><button className="header-theme-button header-notification-button" type="button" aria-label="Notificações" title="Notificações" onClick={()=>setHeaderNotificationsOpen(v=>!v)}><ManagementIcon name="alert" size={19}/>{receptionPanel.waitlist>0&&<span className="header-notification-count">{receptionPanel.waitlist}</span>}</button>{headerNotificationsOpen&&<div className="header-notification-popover"><strong>Notificações</strong>{receptionPanel.waitlist>0?<button type="button" onClick={()=>{setHeaderNotificationsOpen(false);setPage('Agenda')}}><ManagementIcon name="alert" size={17}/><span><b>{receptionPanel.waitlist} {receptionPanel.waitlist===1?'aluno':'alunos'} na lista de espera</b><small>Verificar vagas e ordem da fila</small></span></button>:<p>Nenhuma pendência no momento.</p>}</div>}</div>}<button className="header-theme-button" aria-label={darkMode ? 'Ativar modo claro' : 'Ativar modo escuro'} title={darkMode ? 'Ativar modo claro' : 'Ativar modo escuro'} onClick={() => setDarkMode(!darkMode)} type="button"><ManagementIcon name={darkMode ? 'sun' : 'moon'} size={19} /></button><div className="header-user"><div className="user-avatar">{(profile.full_name || 'A')[0]}</div><div className="user-info"><strong>{profile.full_name}</strong><span>{isAdmin ? 'Gestão' : 'Recepção'}</span></div></div></div></header><main className="app-content">
      ...
      {!isAdmin && page === 'Início' && <ReceptionDashboard profile={profile} panel={receptionPanel} loading={receptionLoading} error={receptionError} updatedAt={receptionUpdatedAt} onRefresh={loadReceptionPanel} onNavigate={setPage} onNewStudent={() => openForm('ALUNO')} onScheduleStudent={openQuickBooking} onRegisterPayment={openQuickPayment} teachers={employees.filter((employee) => employee.status === 'ATIVO' && employee.teacher_id && employee.teacher_status === 'ATIVO' && (employee.profile?.role === 'PROFESSOR' || String(employee.position || '').toLowerCase().includes('professor'))).map((employee) => ({ ...employee, id: employee.teacher_id }))} onChangeTeacher={changeAppointmentTeacher} hideHeaderBell notificationOpen={headerNotificationsOpen} onNotificationOpenChange={setHeaderNotificationsOpen} />}
      {profile.role === 'RECEPCAO' && page === 'Lista de espera' && <ReceptionWaitlist focus={waitlistFocus} />}
      {page === 'Agenda' && (()=>{const today=todayIso();const selectedDate=agendaDate||today;const dayItems=appointments.filter(a=>a.appointment_date===selectedDate&&String(a.status||'').trim().toUpperCase()!=='CANCELADO'&&(!agendaTeacher||a.teacher_id===agendaTeacher)&&(!agendaStatus||a.status===agendaStatus));const dayWaitlist=agendaWaitlist.filter(w=>w.appointment_date===selectedDate&&w.status==='AGUARDANDO');const teacherName=(id)=>employees.find(e=>e.teacher_id===id)?.profile?.full_name||'Professor a definir';const statusLabel=(s)=>s==='CONFIRMADO'?'Aguardando presença':s==='REALIZADO'?'Presença confirmada':s==='FALTOU'?'Falta registrada':s==='AUSENTE'?'Falta registrada':s==='CANCELADO'?'Cancelado':s;return <section className="students-page live-data-page agenda-redesign"><div className="agenda-title-row agenda-title-validated"><div><h2>Agenda do dia</h2><p>Gerencie os agendamentos e a lista de espera.</p></div><div className="agenda-top-date"><button type="button" aria-label="Dia anterior" onClick={()=>{const d=new Date(selectedDate+'T12:00:00');d.setDate(d.getDate()-1);setAgendaDate(d.toISOString().slice(0,10))}}>‹</button><label><ManagementIcon name="calendar" size={18}/><input type="date" value={selectedDate} onChange={e=>setAgendaDate(e.target.value)}/><strong>{selectedDate===today?'Hoje, ':''}{new Date(selectedDate+'T12:00:00').toLocaleDateString('pt-BR',{day:'2-digit',month:'long',year:'numeric'})}</strong></label><button type="button" aria-label="Próximo dia" onClick={()=>{const d=new Date(selectedDate+'T12:00:00');d.setDate(d.getDate()+1);setAgendaDate(d.toISOString().slice(0,10))}}>›</button></div><button className="dashboard-primary-action" type="button" onClick={openQuickBooking}>+&nbsp; Agendar aluno</button></div><div className="agenda-filterbar agenda-filterbar-compact"><label>Professor<select value={agendaTeacher} onChange={e=>setAgendaTeacher(e.target.value)}><option value="">Todos os professores</option>{employees.filter(e=>e.teacher_id&&e.teacher_status==='ATIVO').map(e=><option key={e.teacher_id} value={e.teacher_id}>{e.profile?.full_name}</option>)}</select></label><label>Status<select value={agendaStatus} onChange={e=>setAgendaStatus(e.target.value)}><option value="">Todos os status</option><option value="CONFIRMADO">Aguardando presença</option><option value="REALIZADO">Presença confirmada</option><option value="FALTOU">Faltou</option></select></label></div><div className="agenda-summary"><article><span><svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><rect x="3.5" y="5.5" width="17" height="15" rx="2.5" stroke="currentColor" strokeWidth="1.8"/><path d="M7.5 3.5v4M16.5 3.5v4M3.5 9.5h17M8 13h.01M12 13h.01M16 13h.01M8 17h.01M12 17h.01" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg></span><strong>{dayItems.length}</strong><small>Agendamentos hoje</small></article><article className="green"><span>✓</span><strong>{dayItems.filter(a=>a.status==='REALIZADO').length}</strong><small>Presenças confirmadas</small></article><article className="amber"><span>◷</span><strong>{dayItems.filter(a=>a.status==='CONFIRMADO').length}</strong><small>Aguardando presença</small></article><article className="red"><span>×</span><strong>{dayItems.filter(a=>a.status==='FALTOU').length}</strong><small>Faltas registradas</small></article><article><span><svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M8.5 11a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7ZM15.8 10a3 3 0 1 0 0-6" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/><path d="M2.8 19.5v-1.2c0-3 2.5-5.3 5.7-5.3s5.7 2.3 5.7 5.3v1.2M15.3 13.2c3.2.2 5.2 2.2 5.2 5v1.3" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/></svg></span><strong>{receptionPanel.waitlist}</strong><small>Lista de espera</small></article></div><div className="agenda-content-grid agenda-content-validated"><div className="agenda-main-card"><div className="agenda-card-head"><div><span className="placeholder-kicker">AGENDA DO DIA</span><h3>{new Date(selectedDate+'T12:00:00').toLocaleDateString('pt-BR',{weekday:'long',day:'2-digit',month:'long',year:'numeric'})}</h3></div><button className="outline-action" onClick={loadAgenda} type="button"><ManagementIcon name="refresh" size={16}/> Atualizar</button></div><div className="agenda-list">{dayItems.length?dayItems.map(item=><article className={`agenda-row status-${String(item.status).toLowerCase()}`} key={item.id}><time>{String(item.start_time).slice(0,5)}</time><div className="agenda-person"><span>{(item.student?.full_name||'A')[0]}</span><div><strong>{item.student?.full_name||'Aluno'}</strong><small>Agendamento confirmado</small></div></div><div className="agenda-teacher"><small>PROFESSOR</small><strong>{teacherName(item.teacher_id)}</strong></div><span className={`agenda-status ${String(item.status).toLowerCase()}`}>{statusLabel(item.status)}</span><div className="agenda-actions">{item.status==='CONFIRMADO'&&<><button className="agenda-presence" type="button" onClick={()=>registerAgendaAttendance(item,'REALIZADO')}>Efetuar presença</button><button className="agenda-absence" type="button" onClick={()=>registerAgendaAttendance(item,'FALTOU')}>Registrar falta</button></>}{item.status==='REALIZADO'&&<button className="agenda-undo-attendance" type="button" onClick={()=>undoAgendaAttendance(item)}>Desfazer presença</button>}{['FALTOU','AUSENTE'].includes(item.status)&&<button className="agenda-undo-attendance" type="button" onClick={()=>undoAgendaAttendance(item)}>Desfazer falta</button>}<button className="agenda-more" type="button" title="Pagamentos do aluno" aria-label="Abrir pagamento do aluno" onClick={()=>openAgendaPayment(item)}>•••</button></div></article>):<div className="agenda-empty">Nenhum agendamento para a data selecionada.</div>}</div></div><aside className="agenda-waitlist-card agenda-waitlist-featured agenda-waitlist-side"><div className="agenda-waitlist-head"><div className="agenda-waitlist-heading"><span className="agenda-waitlist-icon">♟</span><div><span className="placeholder-kicker">LISTA DE ESPERA</span><h3>{dayWaitlist.length} aluno{dayWaitlist.length===1?'':'s'} aguardando vaga</h3></div></div><button className="outline-action" type="button" onClick={()=>setPage('Lista de espera')}>Ver lista completa</button></div><div className="agenda-waitlist-groups">{dayWaitlist.length?[...new Set(dayWaitlist.map(item=>String(item.start_time).slice(0,5)))].map(time=><section className="agenda-waitlist-group" key={time}><div className="agenda-waitlist-group-head"><strong>{time}</strong><span>{dayWaitlist.filter(item=>String(item.start_time).slice(0,5)===time).length} na lista</span></div><div className="agenda-waitlist-list">{dayWaitlist.filter(item=>String(item.start_time).slice(0,5)===time).map(item=><div className="agenda-waitlist-row" key={item.id}><span className="agenda-waitlist-position">{item.position||1}</span><span className="agenda-waitlist-avatar">{(item.student?.full_name||'A')[0]}</span><div className="agenda-waitlist-person"><strong>{item.student?.full_name||'Aluno'}</strong><small>{item.position||1}º na fila</small>{item.student?.phone&&<span className="agenda-waitlist-phone">{item.student.phone}</span>}</div><button className="agenda-copy-phone" type="button" onClick={()=>copyWaitlistPhone(item.student?.phone)} title="Copiar telefone" aria-label={`Copiar telefone de ${item.student?.full_name||'aluno'}`}><svg aria-hidden="true" viewBox="0 0 24 24" fill="none"><rect x="8" y="8" width="11" height="11" rx="2.2" stroke="currentColor" strokeWidth="1.8"/><path d="M16 8V6.5A2.5 2.5 0 0 0 13.5 4h-7A2.5 2.5 0 0 0 4 6.5v7A2.5 2.5 0 0 0 6.5 16H8" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/></svg></button></div>)}</div></section>):<div className="agenda-waitlist-empty">Nenhum aluno na lista de espera para este dia.</div>}</div></aside></div></section>})()}
