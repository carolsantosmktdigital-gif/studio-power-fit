...
function AppShell({ profile, onLogout }) {
  ...
  return <div className={`app-shell dashboard-shell ${darkMode ? 'theme-dark' : 'theme-light'} ${isAdmin ? 'is-admin' : 'is-reception'}`}>
    ...
    <nav className="management-mobile-nav" aria-label="Navegação da gestão">{navItems.map((item) => <button key={item} className={page === navigationPage(item) ? 'active' : ''} onClick={() => setPage(navigationPage(item))} type="button"><span>{item === 'Alunos' ? '◉' : item === 'Professores' || item === 'Funcionários' ? '♟' : item === 'Auditoria' || item === 'Relatórios' ? '◷' : item === 'Pagamentos' || item === 'Financeiro' ? '▣' : item === 'Agenda' || item === 'Agendamentos' ? '▢' : '⌂'}</span><small>{item === 'Funcionários' ? 'Equipe' : item === 'Agendamentos' ? 'Agenda' : item}</small></button>)}</nav>
