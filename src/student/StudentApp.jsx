...
function StudentApp({ profile, onLogout }) {
  ...
  const studentPlanLabel = formatStudentPlan(student?.payment_plan || latestPayment?.payment_type)
        ...
        <section className="student-content">
          {page === pages.HOME && (<>
            <section className="student-welcome">
              <div><span className="student-kicker">SEU ESPAÇO POWER FIT</span><h1>{greeting}, <strong>{firstName}!</strong></h1><p>Seu treino começa aqui. Já agendou seu horário?</p><span className="student-welcome-plan"><small>SEU PLANO</small><strong>{studentPlanLabel}</strong></span></div>
...
function formatStudentPlan(value) {
  const plan = String(value || '').toUpperCase().replace(/[\s_-]/g, '')
  if (plan === 'MENSALIDADE' || plan === 'MENSALISTA') return 'Mensalista'
